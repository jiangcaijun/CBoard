...
function showMessage(message) {
    ...
    console.log(map_data.status);
    console.log(map_polygons["a12194e7d25ea6ab151234f367469b6d"].etFillColor())
    if(map_data.status == 11){
        ...
        var map_change_id = JSON.parse(map_data.data);
        map_change_id.forEach((v,i)=>{
            for(var item in map_polygons ){
                if(item == v.id){
                    // map_polygons[item].setFillColor("#f2ff21")
                    // console.log(map_polygons[item].getContent())
                    console.log(map_polygons[item])
                    console.log(v.id)
                }
            }
        })
...
function tableToExcel(){
    ...
    $.post('ext/getDataset.do',{
        ...
        let str = `六边形经度,六边形纬度,所在城市,未接单,可接单闪送员,接收单量,支付单量,该区域接单率,该区域中心位置_1,该区域中心位置_2,该区域中心位置_3\n`;
