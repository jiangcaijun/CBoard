...
var MonitUrl = 'bmap/data4EachCity.do';
...
var thisUrlDay = document.URL;
var websorketDayMontion = new WebSocket(websorketApi);

window.onhashchange=function(){
    console.log(document.URL);
    if(document.URL !== thisUrlDay){
        console.log("地址栏发生变化");
...
websorketDayMontion.onopen = function(message) {
    console.log(websorketDayMontion.readyState)
};
websorketDayMontion.onclose = function(message) {
    clearInterval(setIntervalTimer)
    console.log('连接已关闭43行')
    websorketDayMontion.close();
};
websorketDayMontion.onmessage = function(message) {
...
console.log(websorketDayMontion.readyState )
//发送消息
function sends(cityIds,cityNam) {
    // console.log(cityIds)
    var data = {};
    if(cityIds == ''){
        data = {
            cityId:"0156,1101,3101,3301,4401,4403,5101",
            cityName:cityNam,
        ...
        data = {
            cityId:cityIds + ',0156',
            cityName:cityNam,
            statusFromMenu:"21,22,30,31,20",
            status:10
        };
    }

    if(websorketDayMontion.readyState == 1){
        websorketDayMontion.send(JSON.stringify(data))
    }
    websorketDayMontion.addEventListener('open', function () {
        websorketDayMontion.send(JSON.stringify(data))
    });
}
sends('','');
function showMessage(message) {
    ...
    console.log(mess.status,montion_data)
    if(mess.status == '21'){
        classSetDate(montion_data[0].city_id,montion_data);
        gaugeChartsDate(montion_data[0].city_id,montion_data);
        if($(".map_box").is(":hidden")  == false){
        ...
        $('#'+'name2_'+mess.cityId).find('.wbf_1').empty();
        $('#'+'name2_'+mess.cityId).find('.wbf_2').empty();
...
$(function () {
    $("#city_name").select2();
    $("#monitor_city_name").select2({
        multiple : true
    });
    $("#monitor_select_weather").select2({
        multiple : true
    });
    $("#monitor_select_weather2").select2({
        multiple : true
    });
...
$(document).ready(function(){
    ajaxThen('0f5a68a2-3e22-11e8-b8e5-2f283bb1c5c7').then((res)=>{
        var html = "",html2="";
        ...
        $.each( citys, function(index, content){
            var dataObj =content ;
            // console.log(dataObj)
            html2 += "<option value='"+ dataObj.city_id +"'>"+ dataObj.city_name +"</option>";
            all_cityis.push(dataObj.city_id);
        });
        $('#monitor_city_name').append(html2);
        // dataServer();
    })
});
...
var select_name = function () {
    ...
    var cityId_1= JSON.parse(localStorage.getItem('cityId_'));
    sends(cityId_1,cityName);
    dataServer()
}
// var loadCount = 0;
// map.addEventListener("tilesloaded",function(){
//     if(loadCount == 1){
//         map.setCenter(new BMap.Point(o.center_lng,o.center_lat));
//     }
...
var showInfo1 = function (polygon_cof1) {
    // console.log(polygon_cof1.hexagon_area_id )
    ...
    for (var key in areas1) {
        ...
        (function () {
            ...
            polygon.addEventListener('click', function (e) {
                // 创建信息窗口对象
                var  ciname = $('#city_name').val();
                var liubainid  = e.target.hexagon_area_id;
                // console.log(liubainid)
                var poiHtml = '';
                var address1 = '';
                var address2 = '';
                var address3 = '';
                ajaxThenParams('9bda36a8-75f5-11e8-ac6b-105ea72ce702',"{city_name:'"+ ciname +"'}").then((res)=>{
                    res.forEach(ele=>{
                        if(liubainid == ele.block_id.substring(0,ele.block_id.length-4)){
                            address1 = ele.name_1 ? ele.name_1 : '该区域检索不全';
                            address2 = ele.name_2 ? ele.name_2 : '该区域检索不全';
                            address3 = ele.name_3 ? ele.name_3 : '该区域检索不全';
                            // console.log(ele)
                        }
                    })
                    poiHtml = '<p>'+ address1  +'</p><p>'+address2 +'</p><p>'+ address3 +'</p>';
                    var con = e.target.infoWindow.content;
                    e.target.infoWindow.content = e.target.infoWindow.content + poiHtml;
                    map.openInfoWindow(e.target.infoWindow, e.target.center_point);
                    e.target.infoWindow.content = con;
                })
            })
            ...
            label.addEventListener('click', function (e) {
                ...
                //         }
                //     })
                //     poiHtml = '<p>中心点POI建筑物(1)'+ address1  +'</p><p>中心点POI建筑物(2)'+address2 +'</p><p>中心点POI建筑物(3)'+ address3 +'</p>';
                //     var con = e.target.infoWindow.content;
                //     e.target.infoWindow.content = e.target.infoWindow.content + poiHtml;
                //     map.openInfoWindow(e.target.infoWindow, e.target.center_point);
                //     e.target.infoWindow.content = con;
